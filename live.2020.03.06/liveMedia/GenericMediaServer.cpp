...
::GenericMediaServer(UsageEnvironment& env, 
	unsigned reclamationSeconds)
	: Medium(env),
	fServerSocket(-1), fServerPort(-1),fReclamationSeconds(reclamationSeconds),
	fServerMediaSessions(HashTable::create(STRING_HASH_KEYS)),
	fClientConnections(HashTable::create(ONE_WORD_HASH_KEYS)),
	fClientSessions(HashTable::create(STRING_HASH_KEYS)),
	fPreviousClientSessionId(0)
...
void GenericMediaServer::incomingClientSocket(int clientSocket, struct sockaddr_in clientAddr) {
...
void GenericMediaServer::incomingConnectionHandlerOnSocket(int serverSocket) {
  ...
  incomingClientSocket(clientSocket, clientAddr);
 }
